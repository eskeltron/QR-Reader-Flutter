...
class Validators {
  ...
  final validarHttp = StreamTransformer<List<ScanModel>, List<ScanModel>>.fromHandlers(
    handleData: ( scans , sink ) {
      ...
      final https = scans.where( (s) => s.tipo == 'http').toList();
    ...
    final validarOther = StreamTransformer<List<ScanModel>, List<ScanModel>>.fromHandlers(
    handleData: ( scans , sink ) {

      final others = scans.where( (s) => s.tipo == 'other').toList();
